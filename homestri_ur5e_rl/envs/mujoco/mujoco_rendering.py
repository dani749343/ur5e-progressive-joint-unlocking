...
class WindowViewer(BaseRender):
    ...
    def _key_callback(self, window, key: int, scancode, action: int, mods):
        ...
        # Switch cameras
            ...
            if self.cam.fixedcamid >= self.model.ncam:
                ...
                self.cam.type = mujoco.mjtCamera.mjCAMERA_FREE
        # Pause simulation
        elif key == glfw.KEY_B and self._paused is not None:
            self._paused = not self._paused
        # Advances simulation by one step.
        elif key == glfw.KEY_N and self._paused is not None:
    ...
    def _create_overlay(self):
        ...
        self.add_overlay(
            topleft, "Render every frame [Z]", "On" if self._render_every_frame else "Off"
        )
        self.add_overlay(
            topleft,
            "Switch camera (#cams = %d)" % (self.model.ncam + 1),
            "[X] (camera ID = %d)" % self.cam.fixedcamid,
        )
        self.add_overlay(topleft, "Contact forces [C]", "On" if self._contacts else "Off")
        self.add_overlay(topleft, "Transparent [V]", "On" if self._transparent else "Off")
        if self._paused is not None:
            if not self._paused:
                self.add_overlay(topleft, "Stop", "[B]")
            else:
                self.add_overlay(topleft, "Start", "[B]")
                self.add_overlay(
                    topleft, "Advance simulation by one step", "[N]"
                )
        self.add_overlay(
            topleft, "Reference frames [M]", "On" if self.vopt.frame == 1 else "Off"
        )
        self.add_overlay(topleft, "Hide Menu [<]", "")
        if self._image_idx > 0:
            fname = self._image_path % (self._image_idx - 1)
            self.add_overlay(topleft, "Capture frame [>]", "Saved as %s" % fname)
        else:
            self.add_overlay(topleft, "Capture frame [>]", "")
